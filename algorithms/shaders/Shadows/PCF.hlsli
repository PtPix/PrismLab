#ifndef RENDERLAB_SHADOWS_PCF_HLSLI
#define RENDERLAB_SHADOWS_PCF_HLSLI
...
//   bool  RL_IsInsideShadowMap(int2 texelCoord)       纹素是否落在阴影图覆盖范围内
//
// 约定：
//   * 可见性 0 = 完全遮挡，1 = 完全可见；
...
// 因此这里不给出半成品实现：M2 补齐 RL_ShadowDepthInMeters(...) 之后再添加。
...
{
    const float shadowDepth = RL_LoadShadowDepth(texelCoord);
    const float receiverDepth = RL_ShadowDepthFromWorld(worldPosition);
    ...
    int sampleCount)
{
    ...
    const float receiverDepth = RL_ShadowDepthFromWorld(worldPosition) - depthBiasNdc;
    ...
    [loop] for (int y = 0; y < side; ++y)
    {
        [loop] for (int x = 0; x < side; ++x)
        {
            ...
            if (!RL_IsInsideShadowMap(sampleTexel))
                continue;

            visible += (receiverDepth <= RL_LoadShadowDepth(sampleTexel)) ? 1.f : 0.f;
            taken += 1.f;
        }
    }
...
#endif // RENDERLAB_SHADOWS_PCF_HLSLI
