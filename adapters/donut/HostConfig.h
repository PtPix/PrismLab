...
// Donut adapter: the shared experiment configuration (window, camera, lighting, scene, render).
//
// A lab does not read the JSON itself: it asks for its own section ("labs" -> "<lab name>") and keeps
// its parameters in the contracts layer. Defaults for the whole host come from
// configs/host/camera_default.json and can be overridden with --config <path>.
