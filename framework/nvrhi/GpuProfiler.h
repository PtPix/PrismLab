...
// result has been read, so no frame is measured with a query that is still in flight. Scopes are
// flat — nesting is not supported, because a pass list is what the UI and the CSV logs need.
