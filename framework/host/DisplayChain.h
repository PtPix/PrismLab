...
//
// 实现由使用者提供（路线图方向 11）：
//   * 实现 IDisplayChain，在 Experiment::Initialize 里创建并把指针写进 ExperimentContext::displayChain；
...
namespace prism::host
{
    ...
    public:
        ...
        virtual Status Record(ExperimentContext& context, nvrhi::ICommandList* commands, const DisplayInput& input) = 0;

        // 显示相关的参数（曝光、Bloom 强度、曲线选择）在宿主的实验面板里显示。
        virtual void BuildUI(ExperimentContext& context) = 0;

        // 输出分辨率变化：重建按输出尺寸计算的资源。
        virtual void OnOutputResized(ExperimentContext& context, const Extent2D& outputSize) = 0;
    };
}
