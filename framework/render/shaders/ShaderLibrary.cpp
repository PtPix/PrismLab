...
namespace prism::gpu
{
        ...
        const ShaderMacroList& macros)
    {
        ...
        const auto signature = ReflectShaderInterface(*m_ShaderFactory, virtualPath, entryName, macros);
        if (m_ReloadBaseline)
        {
            auto old = m_ReloadBaseline->find(key);
            if (old != m_ReloadBaseline->end() && (signature.empty() || old->second.empty() || signature != old->second))
            {
                m_LastError = std::string("shader interface changed or reflection unavailable; rebuild the application: ") + virtualPath;
                return nullptr;
            }
        }
    ...
    void ShaderLibrary::Register(ShaderReloadClient* client)
    { if (client && std::find(m_Clients.begin(), m_Clients.end(), client) == m_Clients.end()) m_Clients.push_back(client); }

    void ShaderLibrary::Unregister(ShaderReloadClient* client)
    { m_Clients.erase(std::remove(m_Clients.begin(), m_Clients.end(), client), m_Clients.end()); }

    Status ShaderLibrary::Reload(std::shared_ptr<donut::engine::ShaderFactory> candidateFactory)
    {
        ShaderLibrary candidate(m_Device, std::move(candidateFactory));
        candidate.m_ReloadBaseline = &m_Interfaces;
        for (auto* client : m_Clients)
        {
            auto status = client->PrepareShaders(candidate);
            if (!status)
            {
                for (auto* prepared : m_Clients) prepared->DiscardShaders();
                m_LastError = status.GetMessage();
                return status;
            }
        }
        for (auto* client : m_Clients) client->CommitShaders();
        m_ShaderFactory = std::move(candidate.m_ShaderFactory);
        m_ShaderCache = std::move(candidate.m_ShaderCache);
        m_Interfaces = std::move(candidate.m_Interfaces);
        m_LibraryCache = std::move(candidate.m_LibraryCache);
        ++m_Generation;
        m_LastError.clear();
        return Status::Ok();
    }
